...
func traceInterceptor(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
	...
	violetProbe.captureMessage(ctx, "in", info.FullMethod, req)
	resp, err := handler(ctx, req)
	if err == nil && resp != nil {
		violetProbe.captureMessage(ctx, "out", info.FullMethod, resp)
	}
...
func buildServer(creds credentials.TransportCredentials) *grpc.Server {
	...
	ugcpb.RegisterUgcstoreServer(s, &violetUgcstoreServer{})
	registerTradeBoxServer(s, newTradeBoxServer())
	registerCompetitionServer(s, newCompetitionServer(ranked))
...
func main() {
	...
	if nncsEnabled {
		...
		violetProbe.record(probeEvent{Kind: "nncs_disabled"})
