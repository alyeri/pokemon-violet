...
// cert_gen creates a deterministic Violet leaf-certificate profile. The CA is
...
const (
	violetTLSHostname  = "t-50e39f8f-lp1.lp1.t.npln.srv.nintendo.net"
	violetGamesyncHost = "gamesync.npln.nintendo.net"
)

func ensureTlsCertificate(certPath, keyPath string) {
	// This process terminates both tenant matchmaking and the separate
	// gamesync.npln.nintendo.net endpoint. The default certificate therefore
	// needs both observed DNS names; the single-host minimal profile is useful
	...
	if !envEnabled("NPLN_REGENERATE_CERT") {
		if existing, err := tls.LoadX509KeyPair(certPath, keyPath); err == nil {
			if err := certificateMatchesProfile(existing, profile); err == nil {
				...
				violetProbe.record(probeEvent{Kind: "tls_certificate_reused", Fields: fields})
				return
			} else {
	...
	violetProbe.record(probeEvent{Kind: "tls_certificate_generated", Fields: fields})
}
...
func certificateProfileSANs(profile string) ([]string, []net.IP) {
	if profile == "minimal" {
		return []string{violetTLSHostname}, nil
	}
	if profile == "observed" {
		return []string{violetTLSHostname, violetGamesyncHost}, nil
	}
	return []string{
			violetTLSHostname,
...
func certificateMatchesProfile(pair tls.Certificate, profile string) error {
	...
	if err := leaf.VerifyHostname(violetTLSHostname); err != nil {
		return fmt.Errorf("leaf does not match %s: %w", violetTLSHostname, err)
	}
