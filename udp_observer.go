...
// udp_observer captures the first datagrams that Violet sends after NPLN
...
func configuredUDPObserverAddresses() []string {
	...
	if raw == "" {
		// Runtime metadata from Violet 3.0.1 shows that the first
		// post-matchmaking datagram is sent to loopback port 34343 rather
		// than the advertised game-session port 443. Keep both observers:
		// 443 preserves the documented session endpoint and 34343 captures
		// the previously missed Pia-stage packet without replying to it.
		raw = "0.0.0.0:443,127.0.0.1:34343"
	}
...
func describeUDPDatagram(protocol string, local, remote net.Addr, payload []byte) map[string]interface{} {
	...
	case "pia":
		...
		// Violet identifies PiaNpln 6.30. Kinnay documents header versions
		...
		describePiaMessage(fields, payload)
	case "stun":
...
// describePiaMessage decodes only the unencrypted structural metadata used by
// Pia 6.16-6.30. Protocol payload bytes are still retained solely in the
// evidence capture and are not printed to the process log.
func describePiaMessage(fields map[string]interface{}, payload []byte) {
	if len(payload) < 29 || payload[4]&0x80 != 0 {
		return
	}
	version := payload[4] & 0x7f
	if version < 11 || version > 13 {
	...
	cursor := 29 // 28-byte packet header followed by message field flags.
	present := payload[28]
	if present&1 != 0 {
		if cursor+1 > len(payload) {
			return
		}
		fields["pia_message_flags"] = int(payload[cursor])
		cursor++
	}
	if present&2 != 0 {
		...
		fields["pia_message_payload_size"] = int(binary.BigEndian.Uint16(payload[cursor : cursor+2]))
		cursor += 2
	}
	if present&4 == 0 || cursor+4 > len(payload) {
		return
	}
	protocol := payload[cursor]
	fields["pia_protocol_id"] = int(protocol)
	fields["pia_protocol_port"] = int(payload[cursor+1])<<16 | int(payload[cursor+2])<<8 | int(payload[cursor+3])
	cursor += 4
	if present&8 != 0 {
		if cursor+8 > len(payload) {
			return
		}
		fields["pia_destination"] = binary.BigEndian.Uint64(payload[cursor : cursor+8])
		cursor += 8
	}

	const monitoringDataProtocol = 0xa4
	if protocol != monitoringDataProtocol || cursor+16 > len(payload) {
		return
	}
	fields["pia_protocol"] = "monitoring_data"
	fields["monitoring_version"] = int(payload[cursor])
	fields["monitoring_data_type"] = int(payload[cursor+1])
	fields["monitoring_flags"] = int(payload[cursor+2])
	fields["monitoring_declared_size"] = int(binary.BigEndian.Uint16(payload[cursor+4 : cursor+6]))
	fields["monitoring_key_id"] = int(payload[cursor+14])
}
